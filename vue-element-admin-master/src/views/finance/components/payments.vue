        ...
        <div class="order">
            <div class="order-list">
                ...
                >
                    ...
                    <el-table-column prop="vendorId" label="供应商名称">
                        <template slot-scope="scope">{{
                    ...
                    <el-table-column prop="amount" label="已付金额">
                        <template slot-scope="scope">{{
                    ...
                    <el-table-column prop="owner" label="采购人员">
                        <template slot-scope="scope">{{
...
<script>
import { getPaymentsList } from "@/api/finance";
import { updatePurchaseStockinSettle, updatePurchaseStockinClosing } from "@/api/purchase";
import { updatePurchaseReturnSettle, updatePurchaseReturnClosing } from "@/api/purchase";
...
export default {
    ...
    methods: {
        ...
        changeSettleStatus(val) {
            ...
            if (type == "CGTH") {
                updatePurchaseReturnSettle(data).then((res) => {
                    if (res.code == 0) {
                        this.getList();
                    }
                });
            } else {
                updatePurchaseStockinSettle(data).then((res) => {
                    if (res.code == 0) {
                        this.getList();
                    }
                });
        ...
        changeClosingStatus(val) {
            ...
            let type = val.id.slice(0, 4);
            if (type == "CGTH") {
                updatePurchaseReturnClosing(data).then((res) => {
                    if (res.code == 0) {
                        this.getList();
                    }
                });
            } else {
                updatePurchaseStockinClosing(data).then((res) => {
                    if (res.code == 0) {
                        this.getList();
                    }
                });
        ...
        getList() {
            this.listLoading = true;
            getPaymentsList(this.listQuery).then((res) => {
