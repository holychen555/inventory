<template>
    <div class="order">
        <div class="order__name">
            <div class="order-title">销售退货单<span @click="goBack" class="back">&lt; 返回</span></div>
        ...
        <div>
            <div class="order-info">
                ...
                <div class="order-info__item">
                    <span class="order-info__name">退货日期：</span>
                    <span class="order-info__value">{{postForm.returnDate | filterDate}}</span>
                </div>
                <div class="order-info__item">
                 ...
                 <div class="order-info__item">
                    <span class="order-info__name">关联出库单编号：</span>
                    <span class="order-info__value">{{postForm.relatedFromId}}</span>
                </div>
                 <div class="order-info__item">
...
<script>
import { getSalesReturnById,updateSalesReturnApproval,deleteSalesReturn } from "@/api/sales";
import { deleteOrderGoods } from "@/api/ordergoods";
import {info} from '../mixins/info';
export default {
    mixins: [info],
    created() {
        getSalesReturnById(this.orderId).then((res) => {
            console.log(res);
            if (res.code == 0) {
    ...
    methods: {
        agree(){
            this.$prompt('请输入通过备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updateSalesReturnApproval({id:this.orderId,approvalStatus: 1,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getSalesReturnById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        disagree(){
            this.$prompt('请输入驳回备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updateSalesReturnApproval({id:this.orderId,approvalStatus: 2,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getSalesReturnById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        deleteOrder() {
            ...
            let p2 = deleteSalesReturn(this.orderId);
            Promise.all([p1, p2])
                .then((res) => {
        ...
        editOrder() {
            this.$router.push({path:'/sales/order/xsth?orderId='+this.orderId});
        },
    },
};
</script>
