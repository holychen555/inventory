<template>
    <div class="order">
        <div class="order__name">
            <div class="order-title">销售订单<span @click="goBack" class="back">&lt; 返回</span></div>
            ...
            <div v-if="status=='status_passed'">
                <el-button size="mini" type="primary" @click="createOrder">生成销售出库单</el-button>
            </div>
        ...
        <div>
            <div class="order-info">
                <div class="order-info__item">
                  ...
                  <div class="order-info__item">
                    <span class="order-info__name">单据日期：</span>
                    <span class="order-info__value">{{postForm.orderDate | filterDate}}</span>
                </div>
                 <div class="order-info__item">
                    <span class="order-info__name">交货日期：</span>
                    <span class="order-info__value">{{postForm.deliveryDate | filterDate}}</span>
                </div>
                 <div class="order-info__item">
                    <span class="order-info__name">客户：</span>
                    <span class="order-info__value">{{showCustomerName(postForm.customerId)}}</span>
                </div>
                 <div class="order-info__item">
                    <span class="order-info__name">销售人员：</span>
            ...
            <div class="goods-list">
                ...
                >
                    ...
                    <el-table-column width="120px" align="left" label="销售单价(元)">
                        <template slot-scope="{ row }">
                            <span>{{ row.goodsPrice }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column width="130px" align="left" prop="goodsSum" label="销售金额(元)">
                        <template slot-scope="{ row }">
...
<script>
import { getSalesById,updateSalesApproval,deleteSales } from "@/api/sales";
...
export default {
    ...
    created() {
        ...
        getSalesById(this.orderId).then((res) => {
            console.log(res);
            if (res.code == 0) {
    ...
    methods: {
        agree(){
            this.$prompt('请输入通过备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updateSalesApproval({id:this.orderId,approvalStatus: 1,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getSalesById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        disagree(){
            this.$prompt('请输入驳回备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updateSalesApproval({id:this.orderId,approvalStatus: 2,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getSalesById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        deleteOrder() {
            ...
            let p2 = deleteSales(this.orderId);
            Promise.all([p1, p2])
                .then((res) => {
        ...
        editOrder() {
            this.$router.push({path:'/sales/order/xsdd?orderId='+this.orderId});
        },
        createOrder() {
            this.$router.push({path:'/sales/order/xsck?relatedId='+this.orderId});
        }
    },
};
</script>
