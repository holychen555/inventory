<template>
    <div class="order">
        <div class="order__name">
            <div class="order-title">销售出库单<span @click="goBack" class="back">&lt; 返回</span></div>
        ...
        <div>
            <div class="order-info">
                <div class="order-info__item">
                 ...
                 <div class="order-info__item">
                    <span class="order-info__name">出库日期：</span>
...
<script>
import { getSalesStockoutById,updateSalesStockoutApproval,deleteSalesStockout } from "@/api/sales";
...
export default {
    ...
    methods: {
        agree(){
            this.$prompt('请输入通过备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updateSalesStockoutApproval({id:this.orderId,approvalStatus: 1,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getSalesStockoutById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        disagree(){
            this.$prompt('请输入驳回备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updateSalesStockoutApproval({id:this.orderId,approvalStatus: 2,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getSalesStockoutById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        deleteOrder() {
            ...
            let p2 = deleteSalesStockout(this.orderId);
            Promise.all([p1, p2])
                .then((res) => {
        ...
        editOrder() {
            this.$router.push({path:'/sales/order/xsck?orderId='+this.orderId});
        },
    },
};
</script>
