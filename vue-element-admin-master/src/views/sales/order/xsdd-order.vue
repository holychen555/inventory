<template>
    <div class="order">
        <div class="order__name">
            <div>
                销售订单<span @click="goBack" class="back">&lt; 返回</span>
            </div>
...
import { getSalesId, addSales,getSalesById ,updateSales  } from "@/api/sales";
            ...
            this.$refs.postForm.validate((valid, fields) => {
                if (valid) {
                    ...
                    if(this.orderId){
                        ...
                        p2 = updateSales(this.postForm);
                    }else {
                        p1 = addOrderGoods({
                            list: this.goodsList,
                            orderId: this.postForm.id,
                        });
                        p2 = addSales(this.postForm);
                    }
                    Promise.all([p1, p2])
                        .then((res) => {
