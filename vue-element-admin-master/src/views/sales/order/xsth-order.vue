<template>
    <div class="order">
        <div class="order__name">
            <div>
                销售退货单<span @click="goBack" class="back">&lt; 返回</span>
            </div>
                            ...
                            <el-col :span="7">
                                ...
                                >
                                    <el-select
                                        ...
                                        filterable
                                        searchable
                                        :disabled="!postForm.customerId"
                                        placeholder="请选择"
                                    >
                                        <el-option
                                            v-for="item in xsckIdList"
                                            :key="item.id"
                                            :label="item.id"
                                            :value="item.id"
                                        >
...
import { getSalesReturnId, addSalesReturn,getSalesReturnById ,getSalesStockoutByCustomerId,updateSalesReturn  } from "@/api/sales";
        ...
        'postForm.customerId': {
            handler(val){
                if(!this.orderId&&!this.relatedId){
                    getSalesStockoutByCustomerId(val).then(res=>{
                        if(res.code==0){
                            console.log(res);
                            this.xsckIdList = res.data;
                        }
                    })
                }
                console.log(val);
            ...
            this.$refs.postForm.validate((valid, fields) => {
                if (valid) {
                    ...
                    if(this.orderId){
                        ...
                        p2 = updateSalesReturn(this.postForm);
                    }else {
                        p1 = addOrderGoods({
                            list: this.goodsList,
                            orderId: this.postForm.id,
                        });
                        p2 = addSalesReturn(this.postForm);
                    }
                    Promise.all([p1, p2])
                        .then((res) => {
