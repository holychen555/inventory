<template>
    <div class="order">  
        <div class="order__name">
            <div class="order-title">预入库单<span @click="goBack" class="back">&lt; 返回</span></div>
            ...
            <div v-if="status=='status_passed'">
                <el-button size="mini" type="primary" @click="createOrder">生成采购入库单</el-button>
            </div>
        ...
        <div>
            <div class="order-info">
                <div class="order-info__item">
                 ...
                 <div class="order-info__item">
                    <span class="order-info__name">到货日期：</span>
                    <span class="order-info__value">{{postForm.arrivalDate | filterDate}}</span>
                </div>
                 <div class="order-info__item">
...
<script>
import { getPreStockinById,updatePreStockinApproval,deletePreStockin } from "@/api/purchase";
...
export default {
    ...
    props: ["orderId"],
    created() {
        getPreStockinById(this.orderId).then((res) => {
            console.log(res);
            if (res.code == 0) {
    ...
    methods: {
        agree(){
            this.$prompt('请输入通过备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePreStockinApproval({id:this.orderId,approvalStatus: 1,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPreStockinById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        disagree(){
            this.$prompt('请输入驳回备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePreStockinApproval({id:this.orderId,approvalStatus: 2,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPreStockinById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        deleteOrder() {
            ...
            let p2 = deletePreStockin(this.orderId);
            Promise.all([p1, p2])
                .then((res) => {
        ...
        editOrder() {
            this.$router.push({path:'/purchase/order/yrkd?orderId='+this.orderId});
        },
        createOrder() {
            this.$router.push({path:'/purchase/order/cgrk?relatedId='+this.orderId});
        }
    },
};
</script>
