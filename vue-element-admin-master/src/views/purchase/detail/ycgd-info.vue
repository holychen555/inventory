<template>
    <div class="order"> 
        <div class="order__name">
            <div class="order-title">预采购单<span @click="goBack" class="back">&lt; 返回</span></div>
            ...
            <div v-if="status=='status_passed'">
                <el-button size="mini" type="primary" @click="createOrder">生成采购单</el-button>
            </div>
...
<script>
import { getPrePurchaseById,updatePrePurchaseApproval,deletePrePurchase } from "@/api/purchase";
...
export default {
    ...
    methods: {
        agree(){
            this.$prompt('请输入通过备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePrePurchaseApproval({id:this.orderId,approvalStatus: 1,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPrePurchaseById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        disagree(){
            this.$prompt('请输入驳回备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePrePurchaseApproval({id:this.orderId,approvalStatus: 2,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPrePurchaseById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        deleteOrder() {
            ...
            let p2 = deletePrePurchase(this.orderId);
            Promise.all([p1, p2])
                .then((res) => {
        ...
        editOrder() {
            this.$router.push({path:'/purchase/order/ycgd?orderId='+this.orderId});
        },
        createOrder() {
            this.$router.push({path:'/purchase/order/cgdd?relatedId='+this.orderId});
        }
    },
};
</script>
