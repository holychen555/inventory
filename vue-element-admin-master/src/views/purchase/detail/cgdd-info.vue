<template>
    <div class="order">
        <div class="order__name">
            <div class="order-title">采购订单<span @click="goBack" class="back">&lt; 返回</span></div>
            ...
            <div v-if="status=='status_passed'">
                <el-button size="mini" type="primary" @click="createOrder">生成预入库单</el-button>
            </div>
...
<script>
import { getPurchaseById,updatePurchaseApproval,deletePurchase } from "@/api/purchase";
...
export default {
    ...
    created() {
        ...
        getPurchaseById(this.orderId).then((res) => {
            console.log(res);
            if (res.code == 0) {
    ...
    methods: {
        agree(){
            this.$prompt('请输入通过备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePurchaseApproval({id:this.orderId,approvalStatus: 1,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPurchaseById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        disagree(){
            this.$prompt('请输入驳回备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePurchaseApproval({id:this.orderId,approvalStatus: 2,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPurchaseById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        deleteOrder() {
            ...
            let p2 = deletePurchase(this.orderId);
            Promise.all([p1, p2])
                .then((res) => {
        ...
        editOrder() {
            this.$router.push({path:'/purchase/order/cgdd?orderId='+this.orderId});
        },
        createOrder() {
            this.$router.push({path:'/purchase/order/yrkd?relatedId='+this.orderId});
        }
    },
};
</script>
