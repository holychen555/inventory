<template>
    <div class="order">
        <div class="order__name">
            <div class="order-title">采购退货单<span @click="goBack" class="back">&lt; 返回</span></div>
        ...
        <div>
            <div class="order-info">
                ...
                <div class="order-info__item">
                    <span class="order-info__name">供应商：</span>
                    <span class="order-info__value">{{showVendorName(postForm.vendorId)}}</span>
                </div>
                 <div class="order-info__item">
                    <span class="order-info__name">关联入库单编号：</span>
...
<script>
import { getPurchaseReturnById,updatePurchaseReturnApproval,deletePurchaseReturn } from "@/api/purchase";
...
export default {
    ...
    methods: {
        agree(){
            this.$prompt('请输入通过备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePurchaseReturnApproval({id:this.orderId,approvalStatus: 1,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPurchaseReturnById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        disagree(){
            this.$prompt('请输入驳回备注', '提示', {
                ...
                cancelButtonText: '取消',
                    ...
                    updatePurchaseReturnApproval({id:this.orderId,approvalStatus: 2,approvalMsg: value}).then(res=>{
                        if(res.code==0){
                            getPurchaseReturnById(this.orderId).then((res) => {
                                console.log(res);
                                if (res.code == 0) {
        ...
        deleteOrder() {
            ...
            let p2 = deletePurchaseReturn(this.orderId);
            Promise.all([p1, p2])
                .then((res) => {
        ...
        editOrder() {
            this.$router.push({path:'/sales/order/cgth?orderId='+this.orderId});
        },
    },
};
</script>
