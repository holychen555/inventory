<template>
    <div class="order">
        <div class="order__name">
            <div>
                采购订单<span @click="goBack" class="back">&lt; 返回</span>
            </div>
...
import { getPurchaseId, addPurchase,getPurchaseById,getPrePurchaseById ,updatePurchase  } from "@/api/purchase";
...
export default {
    ...
    created() {
        ...
        // 存在relatedId即为从预采购单生成
        if(this.relatedId){
            getPrePurchaseById(this.relatedId).then((res) => {
                console.log(res);
                if (res.code == 0) {
                    const {
                        deliveryDate,
                    ...
                    this.postForm.deliveryDate = deliveryDate;
                    this.postForm.orderDate = orderDate;
            ...
            this.$refs.postForm.validate((valid, fields) => {
                if (valid) {
                    ...
                    if(this.orderId){
                        ...
                        p2 = updatePurchase(this.postForm);
                    }else {
                        p1 = addOrderGoods({
                            list: this.goodsList,
                            orderId: this.postForm.id,
                        });
                        p2 = addPurchase(this.postForm);
                    }
                    Promise.all([p1, p2])
                        .then((res) => {
