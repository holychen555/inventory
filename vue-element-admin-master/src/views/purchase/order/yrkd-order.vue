<template>
    <div class="order">
        <div class="order__name">
            <div>
                预入库单<span @click="goBack" class="back">&lt; 返回</span>
            </div>
...
import { getPreStockinId, addPreStockin,getPreStockinById,getPurchaseById ,updatePreStockin  } from "@/api/purchase";
...
export default {
    ...
    data() {
        ...
        return {
            ...
            rules: {
                ...
                arrivalDate: [{ required: true, validator: validateRequire }],
    ...
    created() {
        ...
        // 存在relatedId即为从采购单生成
        if(this.relatedId){
            getPurchaseById(this.relatedId).then((res) => {
                console.log(res);
                if (res.code == 0) {
                    const {
                        vendorId,
    ...
    methods: {
        submit() {
            this.$refs.postForm.validate((valid, fields) => {
                if (valid) {
                    this.postForm.arrivalDate = parseTime(
                        this.postForm.arrivalDate,
                        "{y}-{m}-{d} {h}:{i}:{s}"
                    );
                    if (!this.checkGoods) {
                    ...
                    if(this.orderId){
                        ...
                        p2 = updatePreStockin(this.postForm);
                    }else {
                        p1 = addOrderGoods({
                            list: this.goodsList,
                            orderId: this.postForm.id,
                        });
                        p2 = addPreStockin(this.postForm);
                    }
                    Promise.all([p1, p2])
                        .then((res) => {
